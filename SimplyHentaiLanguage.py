...
#多线程获取image    
def fetchPageImg(imageURLQueue, ignore):
    ret = True
    time.sleep(numpy.random.random())
    while True:
        try:
...
#爬虫入口
def crawler(urlroot, url, path, threads, hide, clean, ignore):
            ...
            if mkdir(path+os.sep+title):
                ...
                if Error == False:
                    ...
                    for _ in range(0, thn):
                        t = ImgThread(fetchPageImg, args=(urlQueue, ignore))
                        ths.append(t)
                    for t in ths:
                        t.start()
                    for t in ths:
...
if __name__ == "__main__":
    ...
    URLSUB = "album/language/"+args.sub
    PATH = 'F:/2017/TMP/Download_SH_'+args.path
    ...
    crawler(URLROOT, URLCRAWLER, PATH, THREADS, HIDE, CLEAN, IGNORE)
