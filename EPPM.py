...
def getPage(url, num):
    ...
    try:
        ...
        #剥离其它部分，仅保留相关列表
        pattern = re.compile('<ul class="e2">(.*?)</ul>', re.S)
        content = re.search(pattern, response).group(0)
        #print(content)

        #逐个提取主题
        pattern = re.compile('<li><a href="(.*?)" target="_blank" class="title">(.*?)</a> <span class="listdate">(.*?)</span></li>', re.S)
        items = re.findall(pattern, content)#item[0]本地页面地址，需加上urlroot这个网站地址前缀，item[2]发布日期，item[1]标题
        #for item in items:
        ...
        while retry > 0:
            ...
            if items:
                ...
                for item in items:
                    ...
                    if content.upper() not in item[1].upper():
                        continue
